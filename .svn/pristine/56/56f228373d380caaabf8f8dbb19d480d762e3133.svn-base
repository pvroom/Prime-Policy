...
import { Modal, ModalController, ModalOptions, LoadingController, ToastController } from 'ionic-angular';
	...
	constructor(public navCtrl: NavController, 
				...
				pltfrm.ready().then(() => {
					...
					if (pltfrm.is('android')) {
						console.log("Home: Running on Android device!");
						this.DevicePlatform = "Android";
					}
					if (pltfrm.is('ios')) {
						console.log("Home: Running on iOS device!");
					...
					for (var i = 0; i < maxRecs; i++) {
						...
						if (data[i].MeetingType == 'Other') {
							...
							if (data[i].EventName == '' || data[i].EventName === null) {
								...
								tempTitle = tempTitle + data[i].FirstName + " " + data[i].LastName;
