...
export class MyApp {
	...
	SetFlyin(eventData) {
		...
		let loading = this.loadingCtrl.create({
			spinner: 'crescent',
			content: 'Switching to fly-in: ' + eventData.DisplayName
		});
		...
		this.databaseprovider.getDatabaseStats(flags, AttendeeID).then(dataFD => {
			...
			if (dataFD['length']>0) {
				...
				this.localstorage.setLocalValue("FlyinMeetingID", eventData.flyinID);
			...
			if (LoginType == '1') {
				...
				this.databaseprovider.getDatabaseStats(flags, AttendeeID).then(dataSF => {
					...
					if (dataSF['length']>0) {
						...
						console.log('AppComponents: Looping through data for Staff fly-ins dropdown');
	...
	initializeApp() {
		this.pltfrm.ready().then(() => {
			...
			this.localstorage.setLocalValue('DefaultSyncStart','2019-03-03T10:00:00Z');
