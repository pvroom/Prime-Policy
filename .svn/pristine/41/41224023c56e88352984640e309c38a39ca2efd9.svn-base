...
export class MeetingDetailsPage {
	...
	ngOnInit() {
		...
		this.databaseprovider.getAgendaData(flags, AttendeeID).then(data => {
			...
			if (data['length']>0) {
				...
				this.databaseprovider.getAgendaData(flags, AttendeeID).then(data => {
					...
					console.log("getAgendaData, Congressional Members/Staff: " + JSON.stringify(data));
					...
					if (data['length']>0) {
						...
						for (var i = 0; i < data['length']; i++) {
							...
							if (data[i].RepresentingType == "0") {
								...
								visDisplayPartyState = data[i].Title + " for " + data[i].RepresentingType + " " + data[i].RepresentingLastName;
							...
							this.CongressionalMemberList.push({
								...
								cmID: "'" + data[i].congressionalMemberID + "|" + data[i].staffID + "'",
		...
		this.databaseprovider.getAgendaData(flags, AttendeeID).then(data2 => {
			...
			console.log("getAgendaData: " + JSON.stringify(data2));
