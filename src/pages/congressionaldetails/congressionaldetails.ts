...
export class CongressionalDetailsPage {
	...
	ngOnInit() {
		...
		this.databaseprovider.getCongressionalData(flags, AttendeeID).then(data => {
			...
			if (data['length']>0) {
				...
				if (data[0].Chamber == "Senate") {
					...
					this.databaseprovider.getCongressionalData(flags, "0").then(data2 => {
						...
						if (data2['length']>0) {
							...
							for (var i = 0; i < data2['length']; i++) {
								...
								this.CommitteeListing.push({
									...
									CommitteeWebsite: data2[i].Website
								});
						...
						console.log("Senate Committee Promise Rejected");
					});
				...
				if (OfficeX == null || OfficeY == null) {
					...
					if (data[0].RoomNumber.length == 4) {
						FloorNumber = data[0].RoomNumber.charAt(1);
					} else {
						FloorNumber = data[0].RoomNumber.charAt(0);
					}
			...
			console.log("Congressional Data Promise Rejected");
		});
